...
# 页面全局配置
# ==========================================
st.set_page_config(page_title="高反隔热涂层 | 第一性物理求解器", layout="wide")

# ==========================================
# 1. 核心材料物理常数库 (严密复折射率体系)
# ==========================================
MATERIAL_DB = {
...
def percus_yevick_Sq(q, d, phi):
    """Percus-Yevick 解析结构因子 (带高频振荡保护)"""
    phi_eff = min(phi, 0.45) # 极限相变截断
...
def calculate_coating_performance(resin_n, resin_mass, resin_solid, resin_density, active_fillers, thickness):
    ...
    for wl in wl_array:
        ...
        if Sigma_t_transport < 1e-10:
            ...
            omega_t = min(max(omega_t, 0.0), 1.0 - 1e-10) # 极致防溢出
...
# 4. Streamlit 网页交互界面
# ==========================================
st.title("🔬 高反隔热涂层 | 第一性物理测算引擎")
st.markdown("内核已固化：**严格 P1 相似性辐射传输 (RTE)** × **Percus-Yevick 能量守恒解耦** × **相函数动态边界折射**。")
...
with col2:
    ...
    with c_b:
        st.metric(label="涂层实际浓度 (PVC)", value=f"{pvc:.1f}%", 
                  delta="突破树脂包覆" if pvc > cpvc else "体系致密", delta_color="inverse" if pvc > cpvc else "normal")
    with c_c:
