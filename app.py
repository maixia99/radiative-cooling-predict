...
# ==========================================
# 1. 核心材料物理常数库 (彻底解锁专属紫外带隙 wl_uv 与 基底纯度 k_base)
# ==========================================
MATERIAL_DB = {
    '无 (不添加)': {'n': 1.0, 'k0': 0.0, 'k_base': 0.0, 'wl_uv': 0.2, 'density': 1.0, 'oa': 0.0},
    '硫酸钡 (BaSO4)': {'n': 1.64, 'k0': 0.0, 'k_base': 1e-6, 'wl_uv': 0.2, 'density': 4.5, 'oa': 12.0},
    '氧化铝 (Al2O3)': {'n': 1.76, 'k0': 0.0, 'k_base': 1e-6, 'wl_uv': 0.2, 'density': 3.9, 'oa': 20.0},
    '氧化锆 (ZrO2)': {'n': 2.15, 'k0': 0.0, 'k_base': 5e-6, 'wl_uv': 0.25, 'density': 5.7, 'oa': 15.0},
    '氧化镧 (La2O3)': {'n': 1.95, 'k0': 0.0, 'k_base': 5e-6, 'wl_uv': 0.25, 'density': 6.5, 'oa': 18.0},
    '金红石钛白粉 (TiO2)': {'n': 2.70, 'k0': 0.08, 'k_base': 1e-5, 'wl_uv': 0.38, 'density': 4.1, 'oa': 18.0}, 
    '高岭土 (Kaolin)': {'n': 1.56, 'k0': 0.01, 'k_base': 2e-5, 'wl_uv': 0.35, 'density': 2.6, 'oa': 40.0}, 
    '碳酸钙/重钙 (CaCO3)': {'n': 1.59, 'k0': 0.005, 'k_base': 1e-5, 'wl_uv': 0.30, 'density': 2.7, 'oa': 15.0},
    '二氧化硅 (SiO2)': {'n': 1.46, 'k0': 0.0, 'k_base': 1e-6, 'wl_uv': 0.2, 'density': 2.2, 'oa': 25.0}
}
...
def calculate_coating_performance(resin_n, resin_mass, resin_solid, resin_density, active_fillers, thickness):
    ...
    v_air = v_total_solid * (effective_porosity / 100.0) / max(1e-5, 1 - effective_porosity / 100.0) if effective_porosity < 99 else 0
    ...
    for f in active_fillers:
        mat_key = f['mat']
        phi_i = f['true_volume'] / max(1e-5, v_total_film)
        fillers_for_rte.append({
            'mat': mat_key,
            'd': f['size'], 'phi': phi_i, 
            'n_p': MATERIAL_DB[mat_key]['n'], 
            'k0': MATERIAL_DB[mat_key]['k0'],
            'wl_uv': MATERIAL_DB[mat_key]['wl_uv'],
            'k_base': MATERIAL_DB[mat_key]['k_base']
        })
        diagnostics.append({
            '材料': mat_key.split(' ')[0],
    ...
    for wl in wl_array:
        ...
        for f in fillers_for_rte:
            d, phi, n_p, k0, wl_uv, k_base = f['d'], f['phi'], f['n_p'], f['k0'], f['wl_uv'], f['k_base']
            if phi <= 0: continue
            r = d / 2.0
            
            # 1. 微观吸收截面：引入材料专属的禁带截止 (wl_uv) 和底色透过率 (k_base)
            k_lambda = k0 * np.exp(- (max(0.0, wl - wl_uv) / 0.05)**4) + k_base
            ...
            p_mod = (p_theta * Sq_eff) / max(1e-10, S_bar)
        ...
        k2 = integral_tir / integral_hemi if integral_hemi > 1e-10 else 0.6
        k2 = min(max(k2, 0.1), 0.95)
        
        # --- 阶段三：RTE (Radiative Transfer Equation) P1 相似性精确解 ---
        Sigma_s_transport = Sigma_s_total * (1.0 - g_total)
        Sigma_t_transport = Sigma_s_transport + Sigma_a_total
        
        if Sigma_t_transport < 1e-10:
            R_internal = 0.0
        else:
            ...
            if denom < 1e-10:
                ...
                R_internal = R_inf * (1.0 - exp_term) / denom 
            
        # 7. Saunderson 表面-内部耦合折损
...
with col1:
    ...
    with st.container(border=True):
        st.subheader("2. 离散相 (填料体系)")
        tabs = st.tabs(["主材 1", "辅材 2", "辅材 3", "辅材 4", "辅材 5"])
        active_fillers = []
        for i, tab in enumerate(tabs):
            with tab:
                # 默认展示氧化铝的高光时刻
                default_mat_idx = 2 if i == 0 else 0 
                mat = st.selectbox("选择粉体材质", list(MATERIAL_DB.keys()), index=default_mat_idx, key=f"mat_{i}")
                if mat != '无 (不添加)':
                    col_a, col_b = st.columns(2)
                    with col_a:
                        mass = st.number_input("添加量 (质量份)", 0.0, 500.0, 60.0 if i==0 else 10.0, 1.0, key=f"mass_{i}")
                    with col_b:
                        size = st.number_input("主体粒径 (μm)", min_value=0.01, max_value=50.0, value=1.0 if i==0 else 1.0, step=0.05, key=f"size_{i}")
                    if mass > 0:
                        active_fillers.append({'mat': mat, 'size': size, 'mass': mass})

    with st.container(border=True):
        st.subheader("3. 施工控制")
        thickness = st.number_input("目标干膜厚度 (μm)", min_value=10, max_value=1000, value=250, step=10)

with col2:
    st.header("📊 辐射场解析与微观诊断")
    
    with st.spinner("🚀 正在激活高维辐射传输流形积分计算..."):
    ...
    st.markdown("### 🎯 光谱与热力学极值")
    m1, m2 = st.columns(2)
    with m1:
        st.metric(label="太阳光全谱综合反射比 (R_solar)", value=f"{R_sol:.2f} %")
    with m2:
        st.metric(label="大气窗口法向发射率 (ε)", value=f"{E_pred:.3f}")
        
    st.divider()
    
    st.markdown("### 🌈 严密物理反射光谱 $R(\lambda)$")
    if wl_array is not None:
        fig, ax = plt.subplots(figsize=(8, 3.5), dpi=120)
        ax.plot(wl_array, R_spectrum, color='crimson', linewidth=2, label="Calculated Reflectance")
        _, I_sun = get_solar_spectrum()
        ax.fill_between(wl_array, 0, I_sun/np.max(I_sun)*100, color='gold', alpha=0.15, label='AM1.5 Solar Irradiance')
    ...
    st.markdown("### 🧪 配方物理状态诊断")
    c_a, c_b, c_c = st.columns(3)
    with c_a:
        st.metric(label="系统吸油临界 (CPVC)", value=f"{cpvc:.1f}%")
    with c_b:
        st.metric(label="涂层实际浓度 (PVC)", value=f"{pvc:.1f}%", 
                  delta="触发干遮盖相变" if pvc > cpvc else "体系致密", delta_color="inverse" if pvc > cpvc else "normal")
    with c_c:
        st.metric(label="有效结构微孔率", value=f"{porosity:.1f}%")

    if diagnostics:
        df_diag = pd.DataFrame(diagnostics)
        st.markdown("**干膜内部相干特征：**")
